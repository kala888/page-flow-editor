...
export default function(G6: any) {
  G6.registerBehavior('dragEdge', {
    ...
    dragEdgeBeforeShowAnchorBySub(subProcessNode: any) {
      const group = subProcessNode.getContainer();
      group.nodes.forEach((a: any) => {
        const aGroup = a.getContainer();
        aGroup.showAnchor();
        aGroup.anchorShapes.forEach((b: any) => b.get('item').showHotpot());
      });
    },
    dragEdgeBeforeShowAnchor(e: any) {
      ...
      this.graph.getNodes().forEach((node: any) => {
        ...
        group.showAnchor();
        group.anchorShapes.forEach((a: any) => a.get('item').showHotpot());
      });
    },
    _updateEdge(item: any, e: any, force: any) {
      const x = e.x;
      const y = e.y;
    ...
    _updateEdgeDelegate(item: any, x: number, y: number) {
      const self = this;
    ...
    _addSubProcessEdge(node: any, e: any) {
