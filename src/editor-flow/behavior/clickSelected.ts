export default function(G6: any) {
  G6.registerBehavior('clickSelected', {
    ...
    _clearSubProcessSelected() {
      const subProcessList = this.graph.findAll('node', (node: any) => {
        if (node.get('model')) {
      ...
      subProcessList.forEach((node: any) => {
        const group = node.getContainer();
        const subGroup = group.subGroup;
        this._clearGroupSelected(subGroup);
      });
    },
    _clearGroupSelected(group: any) {
      const selected = group.findAll((subGroup: any) => {
    ...
    _clearSelected() {
      let selected: Array<Node> = this.graph.findAllByState('node', 'selected');
