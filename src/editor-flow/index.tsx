...
import { IDefaultModel, IProjectModel, ISelectData } from './types';
...
> {
  ...
  initEvents() {
    this.graph.on('afteritemselected', items => {
      ...
      if (items && items.length > 0) {
        ...
        this.setState({ selectedModel: this.state.projectModel });
      }
    });
  ...
  onItemCfgChange(key, value) {
    ...
    if (items && items.length > 0) {
      ...
      const canvasModel = { ...this.state.projectModel, [key]: value };
      this.setState({ selectedModel: canvasModel });
      this.setState({ projectModel: canvasModel });
    }
  }
  ...
  render() {
    const height: number = this.props.height || 400;
    const { isView, mode, groups } = this.props;
    const { selectedModel } = this.state;
    const readOnly = mode !== 'edit';

    // @ts-ignore
    return (
      <LangContext.Provider value={{ i18n: locale.zh }}>
